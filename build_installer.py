...
def _find_vigem_msi() -> Path:
    try:
        import vgamepad
    except ImportError as exc:
        raise RuntimeError("vgamepad is not installed in the current Python environment.") from exc
...
def _write_iss(version: str, vigem_msi: Path) -> None:
...
Source: "{_inno_path(vigem_msi)}"; DestDir: "{{app}}\\drivers"; DestName: "ViGEmBusSetup_x64.msi"; Flags: ignoreversion
...
def main() -> int:
    ...
    try:
        ...
        vigem_msi = _find_vigem_msi()
        _write_iss(version=args.version, vigem_msi=vigem_msi)
