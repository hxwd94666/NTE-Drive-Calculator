...
- 当前 schema：发行静态 v27、公共共享 v2、账号私有 v34。升级时同步常量、追加迁移、测试和本文件。
...
玩法、层/半场和怪物 ID 是附加证据。通常只有一个完整配置符合才投影环境和可唯一分配的身份；黑之书与
无首铁驭同生命且缺怪物 ID 时例外默认黑之书，并在当前范围显式标红“歧义”。原始逐击不变。
自动候选仅含当前/下一轨外、争锋两个高难对象、四类材料养成副本和异象追猎单 Boss；不含异象巡礼或普通
