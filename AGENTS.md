...
- 角色、弧盘、技能和其他养成页共用 `ProgressionStaminaService`，不得各自实现材料或体力算法。猎人等级
  10/20/30/40/45/50/55 分别解锁鉴别等级 1–7；鉴别等级 3 起只允许显式下调一级。计算先扣除已持有材料，
  再在当前猎人/鉴别等级可用且具有确定正整数产出的副本档位中求最低体力。缺少正式或用户提供的单次产出时
  必须返回 `partial/unavailable` 和材料 ID，不显示伪造总量；多材料同次掉落只计一次副本次数。
...
面板；该副本成为派生分析和边际计算的当前权威基线，原始冻结快照只作审计、恢复与物化来源。进入边际页
而未改变草稿时收益必须为零，后续草稿只与当前生效副本比较。未保存配装覆盖的旧副本继续组合本场原始冻结
装备。恢复原始只切换生效指针，不删除修改副本。显式
...
战报顶部四项总览中的总伤害/DPS在完整轴上使用“聚合伤害 - 权威超杀 + 已计入有效伤害的全轴生命上限
结算”；长页不再重复六项时段指标。伤害构成同时显示块内占比与全队占比，细分只合并同角色 A/E/Q 各段，
QTE、G、闪避/格挡反击、被动以及各种环合、DOT、附着物、倾陷和生命结算保持独立。Buff、逐击和边际表只
保留决策列，原始证据、置信度、缺口和说明进入详情；Buff 伤害覆盖纳入由覆盖逐击联动的生命上限结算。
