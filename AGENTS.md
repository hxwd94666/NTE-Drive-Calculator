...
- 当前 schema：发行静态 v29、公共共享 v2、账号私有 v36。升级时同步常量、追加迁移、测试和本文件。
...
游戏当前空幕/驱动，并读取这些角色当时角色页有效养成（含好感度 10 级、具体觉醒选择和技能等级），一次性
物化副本；不读取配装槽位。捕获与持久化层不得从聚合摘要推测暴击、Buff/Debuff、护盾、治疗、完整队伍、
敌人实例或场景；分析层的静态推算和用户声明必须保持独立来源与置信度，不冒充实测。实时悬浮窗只在当前采集会话且设置启用时显示；结束并生成战报、
...
保留活动指针。启用副本时以其等级、突破、觉醒选择、好感度、技能、弧盘和所选边际配装重建面板；未保存
配装覆盖的旧副本继续组合本场原始冻结装备。恢复原始只切换生效指针，不删除修改副本。显式“同步到角色
页”只写养成字段，绝不写入战报装备或 UID。“从角色页同步（不含空幕驱动）”只覆盖养成并保留副本已选
配装；“从角色页同步（含空幕驱动）”还把角色页游戏当前空幕/驱动复制为本场独立配装覆盖，不保留活动
指针，也不产生任何角色页写回。
