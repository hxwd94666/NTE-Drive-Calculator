...
2.1.0 先交付独立的“战报”一级页面，只使用当前 nte-core CLI 已开放的战斗摘要能力建立以下
闭环：

1. 采集一场聚合战报；深渊记录上下半和层数，非深渊暂标记为未知场景；
2. 固定上半、下半及各自四名角色；
...
BR0～BR5 基于当前聚合摘要完成低等级收益与配装闭环；BR6 随合作方正式逐击接口交付升级事实
质量，不依赖个人本地 JSON 文件。
...
| 阶段 | 状态 | 说明 |
| --- | --- | --- |
| BR0 contract 与摘要口径 | 当前方案 | 本文 `0.7.0`，本地样本仅作研究证据，不是产品入口 |
| BR1 schema 与摘要快照 | 已实现已验证 | v13、原始 JSON、幂等、100/50 retention 和页面状态自动化已通过 |
| BR2 摘要持久化与历史入口 | 已实现已验证 | 自动入库、账号防护、恢复和历史管理自动化已通过；真机继续验收 |
