...
> 文档版本：`0.3.0`
> 初始整理日期：2026-08-04
> 当前执行方案：[2.1.0 战报统计与收益画像开发方案](2.1.0-battle-report-development-plan.md)
> 上游依赖：[2.1.0 nte-core 合作开发需求](2.1.0-nte-core-collaboration-requirements.md)
...
2.1.0 调整为“战报优先”：先用现有 nte-core 战斗摘要和合作方历史 JSON 建立战报页面，产出
实测技能/渠道占比、`RoleBenefitProfile`、单角色综合边际和实测伤害配装权重。独立的手工编队、
最佳四人和最佳八人搜索暂缓，但本文定义的队伍、贡献和外层分配 contract 继续作为长期边界。

战报具体 schema、方法、页面和阶段以
[2.1.0 战报统计与收益画像开发方案](2.1.0-battle-report-development-plan.md) 为准；本文不重复
定义传输和导入实现。
...
合作方完整逐击 RPC 交付前，先利用现有 nte-core 摘要和合作方历史 JSON 完成战报收益闭环。
...
- 角色页保留现有直伤边际，并新增综合边际；
- 按战报中每个 GE 的实际伤害占比计算分来源、分渠道和综合边际；
- 从实测收益画像生成单角色综合权重；
...
- 收集多个单角色 Top-K；
- 外层解决装备 UID 冲突和计算保留锁；
- 按各角色自身收益画像比较配装前后收益率；角色争夺装备时再乘所在半场实测伤害权重；
- 展示装备 UID、角色/渠道收益变化和方案差异；
- 只生成 Preview，不自动保存。

完成标准：锁定方案不可被改变；候选真实 UID 不重复；收益比较不重新读取移动中的当前快照。

S0～S3 是合理短期目标。完整逐击 RPC 未开放时，历史 JSON 提供完整事实，现有 stdio 提供实时
和最终聚合；两者经同一 contract 进入收益画像。没有兼容战报时才使用明确标记的默认画像。
...
- 将历史 JSON 逐击入口平滑替换/扩展为分页 `battle.get_axis`；
- 导入状态区间和完整战斗上下文；
- 对伤害来源、覆盖率和公式模型做校准；
- 输出 T2/T3 质量；
- 建立战斗记录保留和清理策略。
...
| 阶段 | 状态 | 说明 |
| --- | --- | --- |
| S0 公共口径与 contract | 文档草案 | 本文 `0.3.0`，战报优先边界已确定 |
| S1 战报导入、统计与实测收益画像 | 未开始 | 见战报开发方案 BR0～BR4 |
| S2 单角色综合收益与配装优化 | 未开始 | 实测画像优先，默认画像回退 |
| S3 多角色唯一分配与方案比较 | 未开始 | 只编排单角色 Preview、锁和 UID |
| M1 上游角色/武器/队伍接入 | 等待上游 | 见合作开发需求文档 |
| M2 完整逐击 RPC 和状态区间接入 | 等待上游 | JSON 先行，RPC 后续无缝接入 |
