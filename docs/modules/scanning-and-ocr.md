...
- 全量扫描的弃置/锁定管理可在管理弹窗中独立选择评估角色；该角色范围随管理配置保存，
  不读取或修改计算页面的角色选择、优先级和平级组；
- 全量扫描与仓库弃置/锁定管理读取当前账号同一份管理配置；
- 完整结果通过 `import_vision_inventory` 提交账号快照；
- 与鉴定共享公开解析契约和应用级热键管理器。

## 数据边界

每次任务冻结账号、generation、截图/模板目录、用户库和弃置/锁定管理配置中的角色范围。
Integration 只解析，不直接写数据库；取消或异常不能提交半成品快照。
