...
- 命名边界：公开技能身份是 `GA_Radio072_Passive_2`，但当前正式能力配置把它绑定到内部 `Passive3`
  资源；两者属于不同命名层，不能据文件夹编号改写被动身份。
- 直接属性语义：无；属于触发/反应/专用逻辑。
- 人工审计：按目标侧 `DamageResist<Element>Base=-8%` 建模；实机战报校验支持区间从同频 QTE 首击
  时刻起且该首击本身消费减抗。按触发角色属性区分，同属性刷新整段 12 秒而不叠加，不同属性独立存在。
