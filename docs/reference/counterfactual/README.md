# 战斗反事实文本盘点

本目录先盘点游戏静态事实与当前实现边界，再据此设计数据库和设置页展示；现在不把人工推测写回静态库。

## 审计基线

下列计数只对应完整战斗数据集 `cn_1_3_13_fork_materials_v36_20260828`，不是所有已安装图鉴包的合并数量。
独立 `reference` 角色目录有自己的 dataset 身份与角色范围，不能把新增图鉴角色自动计入本战报反事实审计。
更新计数时以对应 `data/manifest.json` 和只读静态库重新核对，不沿用旧版手填数字。

- 角色：25
- 角色技能目录：92
- 角色被动（逻辑角色去重）：44
- 觉醒/共鸣效果：184
- 可配置战斗效果：453
- 规范化 Buff / GE：3053
- Buff 属性修改：565
- Buff 触发关系：1271
- 来源到 Buff / GE 绑定：425

## 审计文件

| 文件 | 审计范围 |
| --- | --- |
| [角色技能](character-skills.md) | A/Z/E/Q/QTE 与技能伤害项 |
| [角色被动](character-passives.md) | 突破被动、触发、状态与专用反应逻辑 |
| [伤害类型](damage-types.md) | 官方字段、持续伤害种类与人工确认边界 |
| [角色觉醒](awakenings.md) | 一至六觉及三/六觉共鸣效果 |
| [弧盘精炼](forks.md) | 每种弧盘五档精炼参数与运行时效果 |
| [空幕套装](equipment-suits.md) | 二件套和四件套效果 |
| [公共与特殊机制](#公共与特殊机制反事实文本盘点) | DOT、环合、倾陷、最大生命、多目标和轴变化 |
| [人工审计结论](review-notes.md) | 用户确认的触发、乘区、消费者和反事实规则 |

## 状态口径

- `结构化重放`：公式输入和消费者已结构化，仍可另外标注运行时状态置信度。
- `结构化估计`：数值公式已建模，但触发、层数、目标或状态来自推算。
- `固定轴估计`：保留真实逐击，用同技能、同类型、面板比值或零增量兜底。
- `已识别待建模`：身份明确，但尚无安全的反事实策略。
- `当前不影响固定轴`：只改变治疗、护盾、移动、资源或未来动作，不能误写为精确零收益。

## 人工审计方式

用户按稳定审计 ID 提供结论，例如 `AWAKEN-1003-Effect2`。每次确认至少记录：触发、状态时序、作用对象、
数值乘区、伤害消费者、是否改变逐击/循环，以及证据来源。人工结论写入 `review-notes.md`，自动重新生成目录
不会覆盖它。

角色常驻被动属于解释原始战斗与候选配置重放所必需的规则，但不是可单独开关的养成项。边际页可以展示被动在
冻结战报中的反事实贡献：正式 Ability/GE 能唯一证明的派生逐击可以直接移除；会改变召唤物生命周期、资源循环、
动作生成或覆盖顺序的部分，只在专用状态模型给出正式事件归属时移除。证据不足时保留真实轴，显示“未量化”及具体
证据缺口；只有已证明当前条件下不产生差异时才显示“不适用”和数值 0。觉醒、技能等级、弧盘、空幕、驱动和面板属性仍按
各自可调整契约生成反事实。
用户已确认的有界估算可作为例外进入结果，但必须标为低置信 `partial`，说明只替换了哪一个机制维度，并将其余时序、
生命周期和派生联动保留为 gap。当前唯一该类例外是娜娜莉 P1：对已识别创生花伤害直接减半，只模拟花数从 5 增至 10，
不模拟发射间隔从 2 秒缩短至 1 秒，也不重建株生命周期。

`team_others` 只在逐击角色身份已知时才能判断“来源角色自身”或“其他队员”。`character_id` 未知的已覆盖逐击保留在
`unavailable` 中并记录受益角色未知 gap；不使用该击生成已量化增量，不创建受益角色行，也不归给 Buff 来源角色。

44 条逻辑角色培养被动现由 `BattleCharacterPassiveService` 提供稳定目录，统一记录突破 2/4 解锁、机制类型、
重放适配器和固定轴策略。确定的常驻、驻场、技能限定和逐击叠层数值直接生成可解释 Buff 区间；浔 Q 终结段
由技能倍率证据层把基础倍率乘 2；法帝娅最大生命汲取继续复用目标生命适配器，并在未解锁对应突破被动时
停止归因和估算。哈尼娅黯星汲取、海月水母弹叠攻、真红盈蓄叠攻按正式逐击向后建立状态。改变目标状态、
资源循环、召唤生命周期或新增逐击的其余被动保留专用适配器身份。固定轴没有足够事件证据时沿用真实原击，
把未知部分留在 `unavailable` 框，不再估为 0。只能确证部分派生逐击时使用 `partial`，完整事件归属使用 `complete`，
已证明当前轴不产生差异才使用 `not_applicable`。

24 条空幕二/四件套现由 `BattleEquipmentSuitService` 提供稳定目录。二件套继续读取正式 `modify_pack`；
四件套按已审计语义区分常驻、Q/E 动作、普通攻击逐击、队伍分属性命中、环合状态、独立叠层和当前不直接改变伤害的
护盾/治疗效果。空幕专用规则命中后不再同时遍历旧 generic Buff links，避免条件属性被常驻、任意技能结束
误触发。目标状态缺少敌方实例键、普通扣血缺少受击者映射时仍明确标作结构化估计。

## 公共与特殊机制反事实文本盘点

> 本表记录当前代码已经存在的重放边界；不是游戏静态资产全集。

| 审计 ID | 机制 | 当前层级 | 已有边界 | 下一审计重点 |
| --- | --- | --- | --- | --- |
| SYSTEM-direct | 直伤与追加攻击 | 结构化重放 | 通用攻击/生命/防御倍率、暴击期望和目标乘区 | 继续逐角色核对特殊倍率 |
| SYSTEM-unbalance-meter | 敌方倾陷条与逐击倾陷值 | 静态值已结构化 | 每个敌方目标独立持有倾陷条；每个逐击按自己的 UnbalValue 削减，所谓额外削减只是该击数值更高 | 补齐按目标的倾陷条时序重放 |
| SYSTEM-heterochrome-meter | 角色环合条、切换与 QTE | 静态值已结构化 | 每个场上角色独立持有环合条；每击按 HeterochromeAdd 增长，满后切换相邻角色触发 QTE 与对应环合 | 补齐场上角色、相邻关系和切换状态机 |
| SYSTEM-nightmare | 噩梦 | 结构化估计 | 层数、独立到期、E/Q施加与五觉生命结算 | 核对所有技能施加点和提前结算 |
| SYSTEM-zankou-dot | 蚀心 / 鸩火 | 结构化估计 | 逐击层数、形态与持续伤害消费者 | 核对所有角色/技能触发和形态边界 |
| SYSTEM-scorch | 浊燃 | 结构化重放 | 等级基础值、静态倍率、层数、50%固定暴击率、环合强度、目标乘区和 DOT 专属最终乘区 | 扩充不同队伍与目标样本 |
| SYSTEM-creation | 创生 | 结构化重放 | 等级基础值、环合强度和目标乘区 | 核对复制创生花等派生来源 |
| SYSTEM-dark-star | 黯星 | 结构化重放 | 等级基础值、环合强度、不暴击和目标乘区 | 核对法帝娅层数与多目标归属 |
| SYSTEM-topple | 倾陷 | 结构化估计 | 逐角色格子求和、敌方UnbalMax档位、穿透和抗性 | 扩充敌人档位和逐角色真实样本 |
| SYSTEM-max-hp | 最大生命减少 | 结构化估计 | 单目标观测结算、安魂曲五觉与法帝娅归因 | 补多目标实例证据 |
| SYSTEM-target | 防御 / 抗性 / 易伤 | 结构化重放 | 用户确认或共享目标属性包 | 补齐目标自动识别和敌方动态Debuff |
| SYSTEM-multi-target | 多目标逐击 | 固定轴估计 | 正式目标ID存在时分别统计；缺ID不串联 | 等待抓包或运行时目标证据 |
| SYSTEM-axis-change | 新增/删除逐击与循环变化 | 固定轴零增量兜底 | 原轴100%保留，不凭空生成动作 | 需要动作生成与能量/冷却状态机 |
| SYSTEM-replacement | 技能替换 / 攻击模组 | 已识别待建模 | 保留已观测逐击并按当前GE识别 | 安魂曲偷取技能、G模组和敌神者等需角色状态机 |
