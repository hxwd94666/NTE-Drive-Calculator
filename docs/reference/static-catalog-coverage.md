...
本清单按产品领域审计 schema v32 发行候选的全部 131 张业务表；已发布静态库仍为 schema v31，直至候选完成原子晋升。“覆盖总览”固定登记并显示这些表的
...
| `fork_permanent_property` | 100 | 合并详情 | 20 件弧盘逐精炼的无条件常驻面板属性；条件项与缺少直接计算证据的项目留在构建审计。 |
