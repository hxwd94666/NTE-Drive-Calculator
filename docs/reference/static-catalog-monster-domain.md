...
中文名猜测怪物 ID。公共页面尚未在本域接线；集成时由“工具 → 游戏资料库”组合根注入
`StaticCatalogMonsterService`，并嵌入 `MonsterCatalogPanel`。
...
| 争锋赏宴 | `feast_stage`、`feast_stage_difficulty`、`feast_*option` | 8 个挑战对象、32 个难度 |
...
发行 manifest 标记 `source_payloads_omitted=true`，所以资料库可展示来源文件和摘要，不声称可查看完整
原始 payload。当前 schema 没有怪物攻击属性/攻击档字段；界面显示“不可用”，不从等级、中文说明
...
关联跳转仅使用 `monster_template_binding`、玩法表中的正式模板 ID，或正式 Unreal 类路径的对象名。
不使用中文名、数值画像相似或战报推断建立身份关系。
