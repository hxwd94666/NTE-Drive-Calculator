...
class StaticCatalogCompositionLifecycleTests(unittest.TestCase):
    ...
    def test_close_failure_does_not_skip_later_owned_callbacks(self) -> None:
        with _patched_domain_root() as harness:
            specs = _build_specs()
            fork = _spec(specs, "fork")
            first = fork.build(None)
            second = fork.build(None)
            first.dispose_error = RuntimeError("first close failed")

            with self.assertRaisesRegex(RuntimeError, "关闭弧盘图鉴"):
                fork.close()
