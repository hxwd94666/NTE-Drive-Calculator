...
class ManualIdentificationInputTests(unittest.TestCase):
    ...
    def _window():
        ...
        methods = controller.IdentificationController
        window._manual_tokens = lambda text: methods._manual_tokens(window, text)
        window._resolve_stat_name = lambda name, percent=False: methods._resolve_stat_name(
            window,
            name,
            percent,
        )
    ...
    def test_manual_stats_accepts_mixed_delimiters_without_values(self):
        ...
        stats = controller.IdentificationController._parse_manual_stats(
            self._window(),
            "攻击力% 暴击，爆伤\n通用伤害; 倾陷 环合",
        )
        self.assertEqual(
    ...
    def test_manual_stats_derives_display_values_from_quality_and_grid_count(self):
        ...
        window = self._window()
        drive_stats = controller.IdentificationController._parse_manual_stats(
            window, "攻击力% 暴伤", quality="Purple", grid_equivalent=2,
        )
        tape_stats = controller.IdentificationController._parse_manual_stats(
        ...
        self.assertEqual(
            {"攻击力": 50.4, "生命值": 672.0},
            controller.IdentificationController._manual_drive_main_stats(3, "Purple"),
        )
