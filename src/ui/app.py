...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self._load_data();
        self._on_log("系统就绪");
        self._maybe_show_quick_start();
        self._maybe_check_updates_on_startup()

    def _load_hotkey_config(self):
        hotkeys = load_hotkey_config(USER_CONFIG_DIR)
        self._hk_capture = hotkeys["capture"];
        self._hk_finish = hotkeys["finish"];
        self._hk_stop = hotkeys["stop"]
    ...
    def _on_edge(self, pos):
        w, h = self.width(), self.height(); m = self._resize_margin; return (
        pos.x() < m, pos.y() < m, pos.x() > w - m, pos.y() > h - m)

    def mousePressEvent(self, e):
        if e.button() == Qt.LeftButton: self._drag_pos = e.globalPosition().toPoint(); self._drag_edges = self._on_edge(
            e.position().toPoint())
    ...
    def mouseMoveEvent(self, e):
        if self._drag_pos and any(self._drag_edges):
            ...
            g = self.geometry();
            L, T, R, B = self._drag_edges
            if L: g.setLeft(g.left() + d.x())
            if T: g.setTop(g.top() + d.y())
            if R: g.setRight(g.right() + d.x())
            if B: g.setBottom(g.bottom() + d.y())
            self.setGeometry(
                g.normalized() if g.width() >= self.minimumWidth() and g.height() >= self.minimumHeight() else self.geometry())
    ...
    def mouseReleaseEvent(self, e):
        self._drag_pos = None; self._drag_edges = (False,) * 4; super().mouseReleaseEvent(e)
    ...
    def _build_ui(self):
        ...
        tl.addStretch()
        for text, oid, slot in [("—", "", self.showMinimized), ("□", "", self._toggle_max),
                                ("✕", "btnClose", self.close)]:
            b = QPushButton(text);
            b.setObjectName(oid);
        ...
        sl.setSpacing(0)
        self.btn_exec = self._nav("⚡  执行", "execute");
        self.btn_equip = self._nav("💎  配装", "equipment")
        self.btn_identify = self._nav("🔍  鉴定", "identify")
        self.btn_blueprint = self._nav("📐  图纸", "blueprint")
        self.btn_config = self._nav("⚙  配置", "config")
        self.btn_my_role = self._nav("👤  角色", "my_role")
        self.btn_settings = self._nav("🔧  设置", "settings")
        for b in [self.btn_exec, self.btn_equip, self.btn_identify, self.btn_blueprint, self.btn_config, self.btn_my_role,
                  self.btn_settings]: sl.addWidget(b)
        sl.addStretch();
        ...
        guide_btn.clicked.connect(self._show_quick_start);
        tbh.addWidget(guide_btn)
        self.status_lbl = QLabel("就绪");
        self.status_lbl.setStyleSheet("color:#6e7681;font-size:12px");
        tbh.addWidget(self.status_lbl)
        guide_btn.setText("使用教程")
        rr.addWidget(tbar)
        self.stack = QStackedWidget()
        self.stack.addWidget(self._page_execute());
        self.stack.addWidget(self._page_equipment())
        self.stack.addWidget(self._page_identify())
        self.stack.addWidget(self._page_blueprint())
        self.stack.addWidget(self._page_config())
        self.stack.addWidget(self._page_my_role())
    ...
    def _nav(self, text, page):
        b = QPushButton(text); b.setCheckable(True); b.clicked.connect(lambda: self._go(page)); return b

    def _go(self, page):
        m = {"execute": 0, "equipment": 1, "identify": 2, "blueprint": 3, "config": 4, "my_role": 5, "settings": 6}
        ...
        t = {"execute": "⚡  执行",
             ...
             "my_role": "👤  角色",
             "settings": "🔧  设置"}
        self.topbar_title.setText(t.get(page, ""))
        for btn in [self.btn_exec,
                    self.btn_equip,
                    self.btn_identify,
                    self.btn_blueprint,
                    self.btn_config,
                    self.btn_my_role,
                    self.btn_settings]: btn.setChecked(False)
        {"execute": self.btn_exec,
         "equipment": self.btn_equip,
         "identify": self.btn_identify,
         "blueprint": self.btn_blueprint,
         "config": self.btn_config,
         "my_role": self.btn_my_role,
         "settings": self.btn_settings}[page].setChecked(True)
        if page == "equipment":
            self._refresh_equip()
        elif page == "identify":
            self._refresh_identify_options()
        elif page == "config":
            self._refresh_config_forms()
        elif page == "blueprint":
    ...
    def _toggle_log(self, enabled):
        ...
        self.log_frame.setVisible(enabled)
        if enabled:
            self._on_log("运行日志已开启")
        else:
            self.log_view.clear(); self.log_view.insertPlainText("(日志已关闭)\n")

    # ── Data
    def _load_data(self):
        try:
            with open(CONFIG_DIR / "roles.json", "r", encoding="utf-8") as f:
                self.roles_db = json.load(f)
            with open(CONFIG_DIR / "sets.json", "r", encoding="utf-8") as f:
                sd = json.load(f).get("sets", {});
                self.sets_db = sd;
                self.all_set_names = list(sd.keys())
    ...
    def _card(self, title):
        c = QFrame();
        c.setStyleSheet("QFrame{background:#161b22;border:1px solid #21262d;border-radius:10px}")
        l = QVBoxLayout(c);
        l.setContentsMargins(20, 16, 20, 16);
        l.setSpacing(8)
        lb = QLabel(title);
        lb.setStyleSheet("font-size:14px;font-weight:600;color:#58a6ff");
        l.addWidget(lb);
        return c
    ...
    def _clear_ss(self):
        ...
        if QMessageBox.question(self, "确认清理", prompt, QMessageBox.Yes | QMessageBox.No,
            ...
            for f in delete_files:
                try:
                    ...
                    pass
            self._refresh_ss();
            logger.success(f"已清理 {len(delete_files)} 个截图")
            if not has_baseline:
...
def _install_feature_methods():
    ...
    from src.features.role import page as my_role_page
