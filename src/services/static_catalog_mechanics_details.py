...
_PLAYER_FORMULAS: dict[
    ...
    "panel_attribute": (
        ...
        (("基础值", "取当前角色的人物、弧盘和其他正式基础值；不取受击目标。"),
         ...
         ("固定加成", "取当前角色装备、账号家具加成与命中时有效 Buff 的同属性固定项。")),
    ),
    "skill_multiplier": (
        "技能倍率",
        "技能倍率 = 等级倍率 ×（1 + 倍率修正合计）",
    ...
    "direct_damage": (
        "直伤计算",
        "最终直伤 = 向下取整[max(0, 技能倍率 × 对应面板 × 增伤 × 暴击 × 防御 × 抗性 × 易伤 × Π独立增伤)]",
    ...
    "critical": (
        "暴击",
        "未暴击 = 向下取整(完整精度伤害)；暴击 = 向下取整[完整精度伤害 × (1 + 暴击伤害)]；期望 = (1-r)×未暴击 + r×暴击",
    ...
    "weave_followup": (
        ...
        (("原伤害实际值", "取被覆纹记录的攻击方真实伤害；正式 DOT 等可记录来源同样保留，不重新计算预计值。"),
         ...
         ("复制比例", "基础为 20%。"),
         ("灵可复制比例加成 / 追击增伤%", "弱点感应启用时各为 10%，未启用时各为 0%；(20% + 10%) × (1 + 10%) = 33%。"),
         ("解释范围", "当前 33% 的拆分解释；追击增伤只乘复制部分。游戏内部拆分尚未独立确认，其他来源不直接代入。"),
         ("强度区", "1 + 20% × 环合强度 / (环合强度 + 180)；环合强度最低按 0 计算。"),
         ("限制", "把强度区减一的结果限制在 0 到 100 之间。"),
         ("最终乘数", "当前核心模型取 1；保留原伤已结算的增伤，不除掉原增伤区，也不将整次覆纹再乘 1.10。"),
         ("伤害属性", "继承每条原伤害的实际属性；受击目标提供该属性抗性。")),
    ),
    "settlement_rounding": (
        "最终伤害向下取整",
        "最终伤害 = 向下取整（不小于零的完整精度伤害）",
...
_PLAYER_FORMULA_STEPS: dict[str, tuple[str, ...]] = {
    ...
    "weave_followup": (
        "记录每条符合条件的实际伤害及其属性，包括正式 DOT 等被记录来源。",
        "读取每条被记录原伤害实际来源角色的环合强度。",
        "基础复制比例加上灵可复制比例加成，乘以一加追击增伤和强度区，再加入强度区减一的限制项。",
        "12 秒结束时逐条追加并最终取整。",
    ),
    "settlement_rounding": (
        "保留公式中间乘区的完整精度。",
