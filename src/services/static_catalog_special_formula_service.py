...
class StaticCatalogSpecialFormulaService:
    ...
    def _creation() -> SpecialFormulaRecord:
        return SpecialFormulaRecord(
            ...
            sections=_formula_sections(
                "单朵创生伤害 = 向下取整[等级基础值 × (1 + 环合强度/600) × 防御区 × 抗性区 × 易伤区]",
                (
                    "从两名环合参与者中选出归属角色。",
                    "按归属角色等级读取创生 16 档基础值。",
    ...
    def _weave() -> SpecialFormulaRecord:
        return SpecialFormulaRecord(
            ...
            chapter="环合",
            title="环合·覆纹",
            subtitle="记录符合条件的灵/咒实际伤害，12 秒结束时按原伤属性追加。",
            aliases=("灵咒", "弱点感应"),
            sections=_formula_sections(
                "覆纹追加 = 原伤害实际值 × [(复制比例 + 灵可复制比例加成) × (1 + 追击增伤%) × 强度区 + 限制(强度区 - 1, 0, 100)] × 最终乘数",
                (
                    "记录每条符合条件的灵/咒实际伤害及其属性；正式 DOT 等被记录来源同样适用。",
                    "基础复制比例加上灵可复制比例加成，再乘以一加追击增伤。",
                    "用原伤害实际来源角色的环合强度计算强度区；复制部分乘强度区后，再加入强度区减一的限制项。",
                    "将原伤害实际值乘以上述合计和最终乘数，继承该条原伤害的属性。",
                ...
                (
                    ...
                    ("复制比例", "基础为 20%。"),
                    ("灵可复制比例加成", "队伍启用灵可“弱点感应”时为 10%，未启用时为 0%。"),
                    ("追击增伤%", "本页按弱点感应启用时 10%、未启用时 0% 拆分；只乘复制部分，不乘后面的限制项。"),
                    ("复制部分合计", "未启用弱点感应：(20% + 0%) × (1 + 0%) = 20%；启用后：(20% + 10%) × (1 + 10%) = 33%。"),
                    ("强度区", "1 + 20% × 环合强度 / (环合强度 + 180)；环合强度最低按 0 计算。"),
                    ("限制", "把强度区减一的结果限制在 0 到 100 之间。"),
                    ("原伤增伤", "保留原伤害已经结算的增伤，不除掉原增伤区；若原伤本身已吃到追击增伤，其收益保留在实际值中。"),
                    ("最终乘数", "当前核心模型取 1；不重复乘入原伤的最终增伤。"),
                ),
                (
                    "覆纹不读取环合 16 档基础伤害表；它以已发生并被正式记录的实际伤害为基数。",
                    "这是当前 33% 结果的拆分解释，尚未独立确认游戏内部按这两个 10% 分步结算；其他追击增伤来源不据此直接代入。",
                    "33% 是复制部分合计，最终追加伤害还受环合强度影响，并非固定为原伤的 33%，也不将整次覆纹再乘 1.10。",
    ...
    def _scorch() -> SpecialFormulaRecord:
        return SpecialFormulaRecord(
            ...
            aliases=("咒暗", "DOT", "持续伤害"),
            sections=_formula_sections(
                "浊燃单跳 = 向下取整[等级基础值 × 结算前层数 × (1 + 环合强度/600) × 防御区 × 抗性区 × 易伤区 × DOT专属最终区 × 暴击分支]",
                (
    ...
    def _nova() -> SpecialFormulaRecord:
        return SpecialFormulaRecord(
            ...
            aliases=("暗魂", "心灵伤害"),
            sections=_formula_sections(
                "单个黯星 = 向下取整[等级基础值 × (1 + 环合强度/600) × 心灵抗性区 × 易伤区]",
                (
    ...
    def _named_dot(
        *,
        ...
        return SpecialFormulaRecord(
            ...
            aliases=aliases,
            sections=_formula_sections(
                "本跳伤害 = 向下取整[技能基础倍率 × 本跳状态系数 × 攻击方对应面板 × 增伤区 × 防御区 × 抗性区 × 易伤区 × 独立最终区 × DOT专属最终区 × 暴击分支]",
                (
