    ...
    def execute(
        ...
        for bp_combo in self._iter_bp_combos(
            ...
            for row_index, column_index in zip(row_ind, col_ind):
                profit = profit_matrix[row_index, column_index]
                if profit < 0:
                    is_valid = False
                    break

                slot, drive = slots[row_index], drives_pool[column_index]
                role = slot["role"]
                ...
                team_score += profit
                team_rank_score += ranking_matrix[row_index, column_index]
